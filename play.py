...
while True:
    ...
    if(count == 0):
        ...
        seconds=audio.info.length   #获取时长
        print('mp3 long:'+convert_time(seconds))
        print('ffmpeg -re -s 1280x720 -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/default_pic/'+pic_files[pic_ran]+'" -i "'+path+'/default_mp3/'+mp3_files[mp3_ran]+'" -vf ass="'+path+'/default.ass" -vcodec libx264 -pix_fmt yuv420p -crf 24 -preset ultrafast -maxrate 1000k -acodec aac -b:a 192k -f flv "'+rtmp+live_code+'"')
